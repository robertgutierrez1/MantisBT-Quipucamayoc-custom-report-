...
foreach( $t_related_custom_field_ids as $t_custom_field_id ) {
	...
	$customlist[] = $t_def['name'];
	$customdata[] = string_custom_field_value( $t_def, $t_custom_field_id, $t_id );
$ino++;
}       # foreach / 0: testcase / 1: codigo de bug / 2:fecha de levantado / 3: fecha de revision / 4: solucion aplicada
?>
...
<tr>
	<td class="bold">
		<?php echo "Fecha de Encontrado:" ?>
	</td>
	<td>
		<?php echo date( $t_date_format, $t_bug->date_submitted ) ?>
	</td>
	<td class="bold">
		<?php echo "Fecha(s) de Revisión:" ?>
	</td>
	<td>
		<?php echo $customdata[3] ?>
	</td>
	<td class="bold">
		<?php echo "Fecha de Levantado:" ?>
	</td>
	<td>
		<?php 
		if( $t_bug->status == 80 || $t_bug->status == 90 ) {
			echo date( $t_date_format, $t_bug->last_updated );
		}
		else{
			echo "No atendido aún";
		}
		?>
	</td>
</tr>
...
<tr>
	<td class="bold">
		<?php echo "Solución Aplicada" ?>
	</td>
	<td colspan="5">
		<?php echo $customdata[4] ?>
	</td>
</tr>
<!--
<tr class="spacer" >
...
-->
<!-fin modificacion/molde original>
<!--
<tr class="bold">
	<td width="16%">
...
$t_related_custom_field_ids = custom_field_get_linked_ids( $t_bug->project_id );
foreach( $t_related_custom_field_ids as $t_custom_field_id ) {
	# Don't display the field if user does not have read access to it EDITAAAAAAAAAAAAAAAAAAAAAAAR
	if( !custom_field_has_read_access_by_project_id( $t_custom_field_id, $t_bug->project_id ) ) {
		continue;
	}
		...
		<?php
			...
			foreach ( $t_attachments as $t_attachment  ) {
				...
				if( $t_attachment['preview'] && $t_attachment['type'] == 'image' && $f_type_page == 'html' ) {
					echo '<br /><img style="max-width:100%; height: auto;" src="', $c_download_url, '" alt="', $t_attachment['alt'], '" /><br />';
				}
			}
		?>
	</td>
